...
class robot:
    ...
    def draw():
        for robot in robots:
            screen.blit(allSprites[robot.sprite][robot.orientation],
                        (robot.tileX*100+robot.pixelX, robot.tileY*100+robot.pixelY))

    def drawRegister():
        for i in range(len(robots[playersTurn].register)):
            screen.blit(allSprites["card"]
            [robots[0].register[i]], (1300+124*i, 0))
    ...
    def moveForwardOne():
        if firstItterationOfRegister:
            if robot.movementForwardValid():
                if robots[playersTurn].orientation == 0:
                    robots[playersTurn].movementDirection = -1
                    robots[playersTurn].movementAxis = "Y"
                    robots[playersTurn].tileY -= 1
                    robots[playersTurn].pixelY = robots[playersTurn].movement * robots[playersTurn].movementTicks
                    ...
                    robots[playersTurn].pixelX = robots[playersTurn].movement * robots[playersTurn].movementTicks
                else:
                    print("error")
...
def drawGameBoard():
    screen.fill((0, 0, 0))
    for tile in listOfTiles:
        screen.blit(allSprites[tile["sprite"]][tile["orientation"]],
                    (tile["tileX"]*100, tile["tileY"]*100))
...
def movementValid():  # Returns True or False
    ...
    # Kan muligens refactoreres ettersom orientasjonene er 0 og 2 eller 1 og 3...
    elif robots[playersTurn].register[register] == "move1" or robots[playersTurn].register[register] == "move2" or robots[playersTurn].register[register] == "move3":
        if robots[playersTurn].orientation == 0:
            for tile in listOfTiles:
                if (tile["tileX"] == robots[playersTurn].tileX) and (tile["tileY"] == robots[playersTurn].tileY):
                    if tile["sprite"] == "wallTile":
...
def move():
    ...
    if firstItterationOfRegister:
        ...
        if robots[playersTurn].register[register] == "leftTurn":
            ...
            if robots[playersTurn].orientation == 0:
                ...
                robots[playersTurn].orientation -= 1
    ...
    if robots[playersTurn].movementLeftInTicks:
        if robots[playersTurn].movementAxis == "X":
            robots[playersTurn].pixelX += robots[playersTurn].movement * \
                robots[playersTurn].movementDirection
        elif robots[playersTurn].movementAxis == "Y":
            robots[playersTurn].pixelY += robots[playersTurn].movement * \
                robots[playersTurn].movementDirection
        else:
            print("movementAxis not X nor Y")
        robots[playersTurn].movementLeftInTicks -= 1

    elif robots[playersTurn].movementLeftInTiles:
        if movementValid():  # Ny linje 27.05 i håp om at det ordner det, den går ikke ut av boksen, men blir stukk når den får gått "1" med enten 2 eller 3, men ikke får fullført fordi den møter på en vegg.
            if robots[playersTurn].movementAxis == "X":
                robots[playersTurn].pixelX = 0
            else:
                # her blir det enten pixelX eller PixelY ut ifra hva som ligger i movementAxis. Hvordan gjør man dette med classer?
                robots[playersTurn].pixelY = 0
            if robots[playersTurn].register[register] == "move1" or robots[playersTurn].register[register] == "move2" or robots[playersTurn].register[register] == "move3":
                # Move these four lines to after the movement pixelX/Y is done, check todo at the bottom!
                if robots[playersTurn].orientation == 1:
                    robots[playersTurn].tileX += 1
                elif robots[playersTurn].orientation == 2:
                    robots[playersTurn].tileY += 1
...
for i in range(5):
    numberChoicesRectList.append(
        allSprites["normalTile"][0].get_rect(center=(730 + i*110, 600)))
numberOfPlayers = 0
playersTurn = 0
selectNewCard = True
...
while mainMenu:
    ...
    screen.blit(myFont.render('How many players?',
                False, (250, 250, 250)), (800, 350))
    for i in range(5):
        screen.blit(normalTile, numberChoicesRectList[i])
        screen.blit(myFont.render(str(i+2), False,
                    (0, 0, 0)), (720 + i*110, 580))
...
while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
            exit()
    # In future people will choose card themselves. For now it picks randomly
    if not(robots[playersTurn].register):
        robot.fillRegisters()

    if gameLoopNumber > 10:
        print("PlayersTurn:", playersTurn, "Register:", register)
        print("TileX:", robots[playersTurn].tileX, "TileY:", robots[playersTurn].tileY, "PixelX:", robots[playersTurn].pixelX, "PixelY:", robots[playersTurn].pixelY,
              "Orientation:", robots[playersTurn].orientation, "Register:", robots[playersTurn].register, "Moved:", robots[playersTurn].moved, "Moving:", robots[playersTurn].moving)
#        move()
        robots[playersTurn].moveForwardOne()
